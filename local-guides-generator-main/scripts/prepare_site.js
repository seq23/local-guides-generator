...
function normalizeToCanonicalPageSetPath(rawPageSetFile) {
  ...
  if (s0.startsWith(needle)) {
    ...
    if (!rel) die(`ERROR: PAGE_SET_FILE resolved to an empty path from: "${rawPageSetFile}"`);
    return `${needle}${rel}`;
  }

  const shorthandPrefixes = ["examples/", "starter_v1.json", "packs/"];
  if (shorthandPrefixes.some(prefix => s0.startsWith(prefix)) || /^[a-z0-9_\-]+_v\d+\.json$/i.test(s0)) {
  ...
  die(
    ...
    'Use a canonical path like: data/page_sets/examples/pi_v1.json'
  );
}
