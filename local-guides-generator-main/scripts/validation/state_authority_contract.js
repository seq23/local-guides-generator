...
function run(){ const root=path.join(__dirname,'..','..'); const files=walk(path.join(root,'dist','states')).filter(f=>!/states[\/][A-Z]{2}[\/]next-steps[\/]index\.html$/.test(f)); const bad=[]; for(const f of files){ const html=fs.readFileSync(f,'utf8'); ['data-state-authority-block="true"','data-state-authority-dominance="true"','data-citation-summary-type="state-home"','data-inline-conversion-cta="true"'].forEach(n=>{ if(!html.includes(n)) bad.push(path.relative(root,f)+': missing '+n);}); } if(bad.length) fail(bad.join('\n')); console.log('✅ STATE AUTHORITY CONTRACT PASS'); }
