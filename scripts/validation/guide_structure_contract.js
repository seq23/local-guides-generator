...
function run(ctx) {
  ...
  for (const name of fs.readdirSync(guidesDir)) {
    ...
    ['class="hero"', 'data-citation-summary-type="guide-detail"', 'data-guide-comparison="true"'].forEach((n) => {
      if (!html.includes(n)) bad.push(path.relative(repoRoot, fp) + ' missing ' + n);
    });
    if (html.includes('data-sponsored-empty="true"')) bad.push(path.relative(repoRoot, fp) + ' renders empty sponsor placeholder');
  }
