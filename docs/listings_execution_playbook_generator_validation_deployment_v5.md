...
### Deferred Active Projects (target: Jan 30, 2026)

These are **active but deferred** items to finish later. Do not start them mid-batch unless explicitly declared as the session objective.

1. **PI only:** Add a state disciplinary lookup adjacent to the directory on PI city pages.
2. **PI only:** Overhaul the cities on the PI vertical and integrate state pages + city pages (PI lawyers work by state).
3. **Design overhaul:** Overhaul the site design.
4. **Repo scaling:** Complete the repo overhaul so one repo supports multiple verticals.
...
# ============================== APPENDIX — TERMINAL SOP (DUMBED DOWN)

This SOP is intentionally simple and linear. It mirrors the exact workflow used in the recent Batch 3 import.

## Goal
Replace an existing live repo with a baseline ZIP safely, validate locally, and push to GitHub.

## Your repo path (use these exact commands)
```bash
...
git status
```
Expected: it prints the local-guides-generator path and says working tree clean.

## 1) Make a backup branch
```bash
git switch -c backup/pre-import
```

## 2) Push the backup branch
```bash
git push -u origin backup/pre-import
```

## 3) Switch back to main and update
```bash
git switch main
git pull
```

## 4) Unzip the baseline ZIP (NOT inside the repo)
```bash
mkdir -p ~/tmp/import_zip
cd ~/tmp/import_zip
unzip -q "$HOME/Desktop/<YOUR_ZIP_NAME>.zip" -d .
ls -la
```
Expected: you see a folder like `local-guides-generator-main/`.

## 5) Copy baseline into the live repo (ignore .git + dist + node_modules)
```bash
rsync -av --delete   --exclude=".git"   --exclude="dist"   --exclude="node_modules"   --exclude="_lkg_snapshot.json"   "$HOME/tmp/import_zip/local-guides-generator-main/"   "/Users/sequoiataylor/Documents/GitHub/local-guides-generator/"
```

## 6) Ensure dist + snapshot are ignored (only if missing)
```bash
printf "
# Build output
/dist/

# Generated LKG snapshot (build artifact)
/_lkg_snapshot.json
" >> .gitignore
```
...
## 8) Confirm what you’re committing
```bash
...
npm run build
```
Expected: build + postbuild validator PASS.

## 10) Commit + push
```bash
git commit -m "Import baseline + validate"
git push
```

## 11) Create a tag (milestone label)
```bash
git tag -a batch3-hardened-baseline -m "Post-Batch-3 hardened baseline"
git push origin batch3-hardened-baseline
```

## 12) Create a local ZIP backup (outside the repo)
```bash
mkdir -p "$HOME/Desktop/Backups"
cd "/Users/sequoiataylor/Documents/GitHub/local-guides-generator"
HASH=$(git rev-parse --short HEAD)
zip -r "$HOME/Desktop/Backups/local-guides-generator_${HASH}.zip" .   -x ".git/*"   -x "node_modules/*"   -x "dist/*"
```

---

## Edge cases (NOT part of the main SOP)

### A) Docs/files regress unexpectedly
If you care about preventing doc regression, use selective staging instead of `git add -A`.

### B) The ZIP unzips into an unexpected folder name
Run `ls -la` and update the rsync source path.

### C) You copied into the wrong repo folder
Run `pwd`. If the path is wrong, do not commit.

### D) You need to fully undo
Because you pushed `backup/pre-import`, you always have a remote restore point.
