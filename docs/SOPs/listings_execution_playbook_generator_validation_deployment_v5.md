...
Generator architecture and pack selection (no repo consolidation assumption).
...
4. [Active Scope — Execution Playbook](#active-scope--execution-playbook)
   - [Active Projects — Target Completion: January 30, 2026](#active-projects--target-completion-january-30-2026)
...
# Execution Playbook — Generator, Validation & Deployment (v5)

Status: ACTIVE (Execution Reference) — Version: v5
...
## SOP — Terminal: Safe Repo Replace + Commit (Step-by-Step)

This is the exact safe flow for replacing a live repo with an imported baseline, without committing build output.
...
### 2) Create a backup branch (pre-change snapshot)
...
### 4) Import baseline into a temp folder and rsync into repo

```bash
mkdir -p ~/tmp/import
cd ~/tmp/import
unzip -q "$HOME/Desktop/<YOUR_ZIP_FILE>.zip" -d .
...
rsync -av --delete \
  ...
  "$HOME/tmp/import/local-guides-generator-main/" \
  "/Users/sequoiataylor/Documents/GitHub/local-guides-generator/"
```

### 5) Ensure dist + node_modules are ignored (only if missing)

```bash
# Add if not already present
printf "\n# Build output\n/dist/\n\n# Generated LKG snapshot (build artifact)\n/_lkg_snapshot.json\n" >> .gitignore

# If they were ever committed in the past, remove from index
git rm -r --cached dist 2>/dev/null || true
git rm --cached _lkg_snapshot.json 2>/dev/null || true
```

### 6) Install + validate ALL packs (hard gate)
...
### 8) Tag this baseline (recommended)

```bash
git tag -a baseline-<short-name> -m "Baseline snapshot"
git push origin baseline-<short-name>
```

### Optional: local zip backup (outside git)

```bash
mkdir -p "$HOME/Desktop/Backups"
HASH=$(git rev-parse --short HEAD)
zip -r "$HOME/Desktop/Backups/local-guides-generator_${HASH}.zip" . \
  -x ".git/*" -x "node_modules/*" -x "dist/*"
```

#### Edge-case guidance (NOT part of main SOP)
- If docs regress during import: stop, restore docs from your locally saved canonical versions via GitHub Web UI after the system changes are merged.
- Never treat imported ZIP docs as authoritative unless the Master Index Addendum says so.
